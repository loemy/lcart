...
@section('title')
  @Lang('Geo') .'|'. @Lang('Geo Add')  
@endsection

@section('page-header')
<h1>@Lang('Geo')<small> @Lang('Geo Add')</small></h1>
    ...
    <div class="col-sm-12">
        <div class="box box-pink">
            <div class="box-header">
               <div class="box-title"> @Lang('Add Geo')</div>
               @role('')
                {!! Form::open(['route' => 'admin.geo.store', 'class'=>'form-horizontal', 'role' => 'form', 'method'=>'post' 'file'=>true]) !!}
                {{ csrf_field() }} 
                   ...
                   @permission('')
                    ...
                    {!! Form::button(['type' =>'submit', 'name' => 'cancel', 'class'=>'btn btn-circle btn-danger ripple', 'data-toggle' =>'tooltip', 'title' =>
                     '{{trans('Cancel')}}' 'route' => 'admin.geo.index', ],<i class="ion ion-ios-close-outline"></i>) !!}
                   @endpermission
               </div>  
            </div><!--box-header-->
            <div class="box-body">
